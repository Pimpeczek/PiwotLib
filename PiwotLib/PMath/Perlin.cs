...
namespace PiwotToolsLib.PMath
{
    public class Perlin
    {
        ...
        #region Fractional Brownian Noise
        ...
        #region RNG
        public static int RandomInt(int x, int y, int resolution = 4, float multiplierStep = 2)
        {
            double val = 0;
            float multiplier = 1;
            float maxValue = (float)Math.Pow(multiplierStep, resolution * resolution);
            for (int dx = 0; dx < resolution; dx++)
            {
                val = 0;
                for (int dy = 0; dy < resolution; dy++)
                {
                    val += multiplier * Noise(x + ((float)dx) / resolution, y + ((float)dy) / resolution, 0);
